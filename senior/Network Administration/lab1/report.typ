...
Связность на каждом физическом сегменте была проверена `ping`; во всех трех сетях получено 0 % потерь.
...
Проверка R1-R2 показала важное свойство маршрутизации: наличие пути только в направлении запроса недостаточно. После добавления маршрута на R1 ICMP-ответ не мог вернуться к `10.0.1.1`; после добавления обратного маршрута на R2 связь восстановилась. При итоговой проверке loopback-интерфейсы всех трех устройств были взаимно достижимы.
...
До отказа `ip route get` на R1 выбирал прямой next hop:
...
После отключения основной маршрут исчез из активной таблицы, и R1 стал использовать резервный next hop `10.0.13.3`. Связь между loopback-интерфейсами сохранилась: 5 из 5 ICMP-пакетов были доставлены, TTL ответа уменьшился с 64 до 63 из-за дополнительного маршрутизатора.
...
Отдельно отмечено различие состояний интерфейса Linux: после отключения одного конца veth второй конец оставался административно `UP`, но имел `NO-CARRIER`, а маршруты через него отображались с признаком `linkdown`. После административного `down` второго конца соответствующие маршруты были удалены из активной таблицы.

= Маршрут по умолчанию

После проверки резервирования линия R1-R2 была включена снова. Специфические маршруты R1 к `10.0.1.2/32` были удалены, после чего `ip route get 10.0.1.2 from 10.0.1.1` вернул `Network is unreachable`.
...
После настройки default route связь R1→R2 восстановилась, итоговая проверка дала 0 % потерь, `traceroute` снова показал один переход.
